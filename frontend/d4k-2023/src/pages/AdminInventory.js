...
const Tab = () => {
    ...
    const { getDrinks, getDrinksAdmin, updateOutOfStock, updateIsHidden, isLoading, hasError, clearError } = DrinkApi()
    ...
    const Switch86 = ({ drink, onSwitchFunction }) => {
        ...
        return (
            <Switch
                ...
                onValueChange={() => update(drink)}
                thumbIcon={({ isSelected, className }) => isSelected ? (<FontAwesomeIcon className={className} icon={faX} />) : (<></>)} />
        )
    }

    const SwitchHidden = ({ drink, onSwitchFunction }) => {
        const [isHidden, setIsHidden] = useState(false)
        useEffect(() => {
            setIsHidden(drink.is_hidden)
        }, [drink.is_hidden])
        ...
        const update = async (drink) => {
            ...
            try {
                response = await updateIsHidden(drink.drink_id, isHidden)
            } catch (error) {
                console.log(error)
            }
    ...
    const renderCell = useCallback((drink, columnKey) => {
        ...
        const handleIsHiddenSwitch = (drink, value) => {
            setDrinks(a => a.map(o => o.drink_id === drink.drink_id ? { ...o, is_hidden: value} : o))
        }
        ...
        switch (columnKey) {
            ...
            case "is_hidden":
                return (<SwitchHidden drink={drink} onSwitchFunction={handleIsHiddenSwitch} />)
            default:
                return cellValue
        }
    }, [drinks])
                    ...
                    <div className="rounded-lg shadow-md">
                        <Table topContent={topContent} topContentPlacement="outside"
                            ...
                            <TableHeader className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                ...
                                <TableColumn key="is_hidden" scope="col" className="py-3 w-1/12">Hidden</TableColumn>
