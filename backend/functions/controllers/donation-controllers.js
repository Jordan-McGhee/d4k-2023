...
const createDonation = async (req, res, next) => {
  ...
  const text = "INSERT INTO donations(username, amount, comments, is_paid, created_at, updated_at) VALUES ($1, $2, $3,  false, NOW(), NOW())";
...
const getDonationsAdmin = async (req, res, next) => {
  const unpaidQuery = "SELECT * FROM donations WHERE is_paid = FALSE ORDER BY created_at ASC";

  const paidQuery = "SELECT * FROM donations WHERE is_paid = TRUE ORDER BY created_at ASC";

  let unpaidResponse; let paidResponse;
...
const updateDonationPaid = async (req, res, next) => {
  ...
  const text = "UPDATE donations SET is_paid = $1, updated_at = NOW() WHERE donation_id = $2 RETURNING *";
...
const updatedDonationAmount = async (req, res, next) => {
  ...
  const text = "UPDATE donations SET amount = $1, updated_at = NOW() WHERE donation_id = $2 RETURNING *";
...
const deleteDonation = async (req, res, next) => {
  ...
  const text = "DELETE FROM donations WHERE donation_id = $1";
...
const closeDonations = async (req, res, next) => {
  ...
  const text = "UPDATE donations SET is_paid = TRUE, updated_at = NOW() WHERE UPPER(username) = UPPER($1) RETURNING *";
